...
function run () {
  ...
  getData("/EducationAndCause39", (data) => {
    const totals = {};
    let key;
    let eduKey;
    let causeKey;
    ...
    data = data.filter((datum) => {
      // Pull the values out of the object
      eduKey = datum["Education"]
      causeKey = datum["Cause of Death"]

      // Filter out unspecified values
      if (
        eduKey === "Not Specified" ||
        causeKey === "Not Specified" ||
        datum["Year"] !== 2008
      ) return false;

      // Sum up totals in education
      if (eduKey in totals) totals[eduKey] += datum["Number in Group"];
      else totals[eduKey] = datum["Number in Group"];
    ...
    data.forEach((datum) => {
      eduKey = datum["Education"]
      datum["Number in Group"] = datum["Number in Group"] / totals[eduKey];
    })

    const eduAndCause = new Viz(data, "#eduCauseViz");
    eduAndCause.bubbleView("Cause of Death", "Education");
  });
}
...
function Viz(data, selector) {
  this.data = data;

  this.root = d3.select(selector);

  this.width = parseInt(this.root.style("width"));
  this.height = parseInt(this.root.style("height"));
}

Viz.prototype.bubbleView = function (xAxis, yAxis) {
  const xLabels = _.uniq(_.pluck(this.data, xAxis));
  const yLabels = _.uniq(_.pluck(this.data, yAxis));

  const elemWidth = this.width / xLabels.length;
  const elemHeight = this.height / yLabels.length;

  const maxRadius = Math.min(elemWidth, elemHeight);

  xLabels.forEach((label, index) => {
    const pos = index * elemWidth + 0.5 * elemWidth;
  ...
  yLabels.forEach((label, index) => {
    const pos = index * elemHeight + 0.5 * elemHeight;
    this.root.append("text")
      .attr({
        y: pos,
      })
  ...
  this.root
    ...
    .append("circle")
    .attr({
      cx: d => maxRadius * xLabels.indexOf(d[xAxis]) + 0.5 * maxRadius,
      cy: d => elemHeight * yLabels.indexOf(d[yAxis]) +  0.5 * elemHeight,
      r: d => Math.sqrt(d["Number in Group"] / Math.PI) * maxRadius * 3,
      "fill-opacity": 0.5,
    })
}